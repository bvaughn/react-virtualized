...
import { createAndInstallFakeTimers } from '@test/helpers'
import expect from 'expect.js'

describe('VirtualScroll', () => {
  const timers = createAndInstallFakeTimers()
  var node = null
  beforeEach(() => {
  ...
  const list = Immutable.fromJS(
    times(100, x => ({
      id: '${x}',
      name: `Name ${x}`,
      email: 'user-${x}@treasure-data.com'
    }))
  )
  ...
  describe('property updates', () => {
    it('should update :scrollToIndex position when :rowHeight changes', () => {
      ...
      expect(list.textContent).to.contain('Name 50')
    ...
    it('should update :scrollToIndex position when :scrollToIndex changes', () => {
      ...
      expect(list.textContent).not.to.contain('Name 50')
    ...
    it('should update scroll position if size shrinks smaller than the current scroll', () => {
      ...
      expect(list.textContent).to.contain('Name 9')
    })
  })
})
