...
| enableFooter      | Boolean             |           | Render the table footer (only the rows)                                                                                                                         |
...
| footerClassName | String | | CSS class to apply to all column footers |
| footerHeight | Number | ✓ | Fixed height of footer row |
| footerRowRenderer | Function | | Responsible for rendering the table footer row given an array of columns. [Learn more](#footerrowrenderer) |
| footerStyle | Object | | Optional custom inline style to attach to table footer columns. |
...
| onFooterClick | Function | | Callback invoked when a user clicks on a table footer. `({ columnData: any, dataKey: string, event: Event }): void`  
 |
...
| ReactVirtualized\_\_Table\_\_footerColumnn        | Footer cell (similar to `tfoot > tr > th`)  |
| ReactVirtualized\_\_Table\_\_footerRow            | Footer row (similar to `tfoot > tr`)        |
...
This is an advanced property.
It is useful for situations where you require additional hooks into `Table` to render additional custom UI elements.
You may want to start by forking the [`defaultTableFooterRowRenderer`](https://github.com/bvaughn/react-virtualized/blob/master/source/Table/defaultFooterRowRenderer.js) function.

This function accepts the following named parameters:

| Property  | Description          |
| :-------- | :------------------- |
| className | Footer class name    |
| columns   | Array of React nodes |
| style     | Footer style object  |
